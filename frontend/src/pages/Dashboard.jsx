...
const Dashboard = () => {
  ...
  const [showAllIncome, setShowAllIncome] = useState(false); //toggle
  const [showAllExpense, setShowAllExpense] = useState(false); //toggle
  ...
  const displayedIncome = showAllIncome
    ...
    : incomeListForDisplay.slice(0, 3); //shows 3 income then a toggle button
  ...
  const handleAddTransaction = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to add transactions error:",
        err?.response || err.message || err,
      );
    } finally {
      setLoading(false);
